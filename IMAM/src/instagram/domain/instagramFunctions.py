...
class InstagramFunctions:
    ...
    def startMachine(self, location ,tagList, likeProbability,videPropability,followingNames, unfolowAmount=100,unfollowDelay=100, comments = ["nice picture","you look amazing"], mode='normal', topic = "entertainment"):
        with smart_run( self.session):
            self.session.join_pods()
            print(location ,tagList, likeProbability,videPropability,followingNames, unfolowAmount,unfollowDelay, comments)
            # if (comments != ''):
            #     print("comments")
            #     print(comments)
            ...
            if (unfolowAmount != '' and unfollowDelay != ''):
                ...
                session.unfollow_users(amount=200, instapy_followed_enabled=True,
                                       instapy_followed_param="nonfollowers",     style="FIFO", unfollow_after=12 * 60 * 60, sleep_delay=601)
